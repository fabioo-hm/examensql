...
```sql
SELECT em.nombre, p.nombre
FROM empleados em
JOIN sucursal s ON em.sucursalid = s.id
...
```sql
SELECT cl.nombre, m.nombre, dp.nombre, p.nombre
FROM clientes cl
JOIN municipio m ON cl.municipioid = m.id
JOIN departamento dp ON m.depid = dp.id
JOIN pais p ON dp.paisid = p.id;
```
...
SELECT DISTINCT
    m.nombre AS municipio
FROM municipio m
JOIN sucursal s ON s.municipioid = m.id
JOIN empleados e ON e.sucursalid = s.id;
```
...
```sql
SELECT p.nombre, COUNT(cl.cliente_id) as total_clientes
FROM pais p
JOIN clientes cl ON cl.municipioid = m.id
JOIN municipio m ON m.depid = dp.id
JOIN departamento dp ON dp.paisid = p.id
GROUP BY (p.id);
```
...
JOIN empleados em ON em.sucursalid = s.id
WHERE NOT EXISTS (
SELECT *
FROM empleados em
WHERE em.puesto='Cajero');
```
...
SELECT em.nombre
FROM empleados em
JOIN sucursal s ON s.id = em.sucursalid
WHERE (
SELECT COUNT(cl.cliente_id)
FROM clientes cl
GROUP BY(cl.municipioid)) > 9;
```
